...
mod llvm {
    ...
    impl<'a> Builder<'a> {
        ...
        pub fn create_function_type(&mut self, return_type: &Type, param_types: &[Type]) -> FunctionType {
            let llvm_types: Vec<rustc_llvm::TypeRef> = param_types.iter().map(|t| t._type).collect();
            let p_types: *const rustc_llvm::TypeRef = llvm_types.as_ptr();
            unsafe {
                FunctionType{ _type: rustc_llvm::LLVMFunctionType(return_type._type, p_types, param_types.len() as libc::c_uint, rustc_llvm::False) }
            }
        }
    ...
    impl ExecutionEngine {
        pub fn new(module: &Module) -> ExecutionEngine {
            unsafe {
                ...
                let engine = ExecutionEngine{ _engine: rustc_llvm::LLVMBuildExecutionEngine(module._module, jit_memory_manager) };
        ...
        pub fn finalize(&self) -> &ExecutionEngine {
        ...
        pub fn get_pointer_to_function(&self, function: &Function) -> *const() {
            unsafe {
                rustc_llvm::LLVMGetPointerToGlobal(self._engine, function._value)
...
fn main() {
    ...
    unsafe {
        ...
        println!("Ejecutando...");
        let f: fn(i32, i32) -> i32 = unsafe { mem::transmute(fptr) };
        let res = f(12, 13);
        println!("res: {}", res);
    }
}
